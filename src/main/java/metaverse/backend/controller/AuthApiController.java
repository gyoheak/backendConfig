...
import metaverse.backend.service.user.UserService;
...
public class AuthApiController {
    ...
    public ResponseEntity<?> validate(@RequestHeader("Authorization") String requestAccessToken) {
        if (!authService.validate(requestAccessToken)) {
            return ResponseEntity.status(HttpStatus.OK).body(HttpStatus.OK); // 재발급 필요X
        } else {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(HttpStatus.UNAUTHORIZED); // 재발급 필요
        }
    }
    // 토큰 재발급
    @PostMapping("/reissue")
