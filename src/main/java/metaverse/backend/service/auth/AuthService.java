package metaverse.backend.service.auth;
...
public class AuthService {
    ...
    public boolean validate(String requestAccessTokenInHeader) {
        ...
        System.out.println("validate " + requestAccessToken);
    ...
    public AuthDto.TokenDto reissue(String requestAccessTokenInHeader, String requestRefreshToken) {
        ...
        // 토큰 재발급 및 Redis 업데이트
        redisSecurityService.deleteValues("RT(" + SERVER + "):" + principal); // 기존 RT 삭제
    ...
    public AuthDto.TokenDto generateToken(String provider, String email, String authorities) {
        // RT가 이미 있을 경우
        if (redisSecurityService.getValues("RT(" + provider + "):" + email) != null) {
            redisSecurityService.deleteValues("RT(" + provider + "):" + email); // 삭제
        }
    ...
    public void saveRefreshToken(String provider, String principal, String refreshToken) {
        redisSecurityService.setValuesWithTimeout("RT(" + provider + "):" + principal, // key
                refreshToken, // value
    ...
    public void logout(String requestAccessTokenInHeader) {
        ...
        if (refreshTokenInRedis != null) {
            redisSecurityService.deleteValues("RT(" + SERVER + "):" + principal);
        }

        // Redis에 로그아웃 처리한 AT 저장
        long expiration = jwtTokenProvider.getTokenExpirationTime(requestAccessToken) - new Date().getTime();
        redisSecurityService.setValuesWithTimeout(requestAccessToken,
                "logout",
                expiration);
    }
}
