...
import metaverse.backend.environment.security.user.UserDetailsImpl;
import metaverse.backend.environment.security.user.UserDetailsServiceImpl;
...
public class JwtTokenProvider implements InitializingBean {
    ...
    private static final String url = "https://localhost:8888";
    ...
    public JwtTokenProvider(
            ...
            RedisSecurityService redisService,
            @Value("${jwt.secret}") String secretKey,
            @Value("${jwt.access-token-validity-in-seconds}") Long accessTokenValidityInMilliseconds,
            @Value("${jwt.refresh-token-validity-in-seconds}") Long refreshTokenValidityInMilliseconds) {
        this.userDetailsService = userDetailsService;
        this.redisSecurityService = redisService;
    ...
    public boolean validateRefreshToken(String refreshToken) {
        try {
//            if (redisSecurityService.getValues(refreshToken).equals("delete")) { // 회원 탈퇴했을 경우
//                return false;
//            }
            Jwts.parserBuilder()
                    .setSigningKey(signingKey)
    ...
    public boolean validateAccessToken(String accessToken) {
        try {
            if (redisSecurityService.getValues(accessToken) != null // NPE 방지
                    && redisSecurityService.getValues(accessToken).equals("logout")) { // 로그아웃 했을 경우
                return false;
            }
            Jwts.parserBuilder()
                    .setSigningKey(signingKey)
